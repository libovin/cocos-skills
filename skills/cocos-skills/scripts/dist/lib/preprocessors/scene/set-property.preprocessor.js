...
async function queryComponentProperties(client, componentUuid) {
    try {
        ...
        const compValue = componentData.value;
        if (!compValue) {
            return [];
        }
...
export const sceneSetPropertyPreprocessor = async (params, client) => {
    ...
    if (!nodeData) {
        throw new ValidationError('scene', 'set-property', 'usage', `节点 "${nodeUuid}" 不存在或无法访问`);
    }
    ...
    if (isNodeProperty) {
        ...
        if (!componentInfo) {
            ...
            throw new ValidationError('scene', 'set-property', 'usage', `节点上未找到组件 "${actualComponentType}"\n可用组件: ${availableTypes}`);
        }
