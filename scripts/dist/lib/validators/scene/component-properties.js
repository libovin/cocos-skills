...
export const componentPropertyMap = new Map(componentProperties.map((comp) => [comp.type, new Set(comp.properties.map((p) => p.name))]));
export function isBuiltinComponent(componentType) {
    return componentPropertyMap.has(componentType);
}
export function isValidProperty(componentType, propertyName) {
...
export function getPropertiesForComponent(componentType) {
