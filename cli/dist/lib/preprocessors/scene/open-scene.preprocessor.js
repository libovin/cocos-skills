 ...
 */
export const sceneOpenScenePreprocessor = async (params, client) => {
    ...
    if (isPathLike(input)) {
        try {
            const response = await client._request('POST', '/api/asset-db/query-uuid', {
                params: [input],
            });
            if (response.success && response.data) {
                ...
                throw new ValidationError('scene', 'open-scene', 'uuid', `无效的资源路径：${input}\n` +
                    `请检查路径是否正确`);
            }
        }
        catch (error) {
    ...
    if (isUuidLike(input)) {
        try {
            ...
            if (!response.success || !response.data) {
                throw new ValidationError('scene', 'open-scene', 'uuid', `无效的 UUID：${input}\n` +
...
//# sourceMappingURL=open-scene.preprocessor.js.map