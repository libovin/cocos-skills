...
export function loadServerUrl(): string | null {
  try {
    ...
    const config: CocosHttpConfig = JSON.parse(content);
 ...
 * @returns Project name or null
 */
function findProjectPath(startPath: string): string | null {
 ...
 */
export function parseServerUrl(serverUrl: string): { host: string; port: number } {
  try {
