...
export class UuidService implements IUuidService {
   ...
   */
  private _decodeUuidInternal(uuid: string): string {
    try {
   ...
   */
  private _compressUuidInternal(uuid: string): string {
    try {
      // 分离 UUID 前缀和内容
   ...
   */
  private _decompressUuidInternal(uuid: string): string {
    try {
      // 分离 UUID 前缀和内容
   ...
   */
  private _originalUuidInternal(uuid: string): string {
    try {
      // 转换成长的 UUID
      const header = uuid.slice(0, 5);
